...
function diagnostic(code, path, message, options = {}) {
  const result = {
    code,
  ...
  if (options.assetId !== void 0) result.assetId = options.assetId;
...
function normalizeRecipe(recipe) {
  ...
  if (recipe.extensions !== void 0) normalized.extensions = sortedRecord(recipe.extensions);
  return normalized;
}
function normalizeRig(rig2) {
  return {
    ...
    anchors: [...rig2.anchors].sort((a, b) => a.id.localeCompare(b.id)),
    expressions: [...rig2.expressions].sort((a, b) => a.id.localeCompare(b.id)),
    clips: [...rig2.clips].sort((a, b) => a.id.localeCompare(b.id)),
    fallbacks: [...rig2.fallbacks].sort(
      (a, b) => a.axis.localeCompare(b.axis) || a.from.localeCompare(b.from) || a.to.localeCompare(b.to)
    ),
    masks: sortedUnique(rig2.masks)
  };
}
...
// packages/schema/src/schemas/asset-pack.schema.json
var asset_pack_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://character-creator.dev/schemas/0.1.0/asset-pack.schema.json",
  type: "object",
  additionalProperties: false,
  required: ["schemaVersion", "id", "version", "engine", "rigFamilies", "assets", "provenance"],
  properties: {
    schemaVersion: { $ref: "common.schema.json#/$defs/schemaVersion" },
    id: { $ref: "common.schema.json#/$defs/id" },
    version: { $ref: "common.schema.json#/$defs/semver" },
    engine: { $ref: "common.schema.json#/$defs/semverRange" },
    rigFamilies: {
      type: "array",
      minItems: 1,
      uniqueItems: true,
      items: { $ref: "common.schema.json#/$defs/rigFamily" }
    },
    assets: {
      type: "array",
      minItems: 1,
      items: {
        type: "object",
        additionalProperties: false,
        required: ["id", "manifest", "contentHash"],
        properties: {
          id: { $ref: "common.schema.json#/$defs/id" },
          manifest: { $ref: "common.schema.json#/$defs/safePath" },
    ...
    provenance: {
      type: "object",
      additionalProperties: false,
      required: ["authors", "license", "source"],
      properties: {
        authors: {
          type: "array",
          minItems: 1,
        ...
        source: { type: ["string", "null"], format: "uri-reference" }
      }
    },
...
// packages/schema/src/schemas/asset.schema.json
var asset_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://character-creator.dev/schemas/0.1.0/asset.schema.json",
  $defs: {
    fallback: {
      type: "object",
      additionalProperties: false,
      required: ["axis", "from", "to", "allowedFor", "forbiddenFor"],
      properties: {
        axis: { enum: ["expression", "view", "clip", "frame", "variant"] },
        from: { type: "string", minLength: 1, maxLength: 128 },
        to: { type: "string", minLength: 1, maxLength: 128 },
        allowedFor: {
          type: "array",
          uniqueItems: true,
          items: { $ref: "common.schema.json#/$defs/assetKind" }
        },
        forbiddenFor: {
          type: "array",
          uniqueItems: true,
    ...
    selector: {
      ...
      required: ["profile"],
      properties: {
        profile: { enum: ["portrait", "full-body", "sprite"] },
        view: { type: "string", minLength: 1, maxLength: 64 },
        expression: { type: "string", minLength: 1, maxLength: 64 },
        clip: { enum: ["idle", "sit", "walk", "run"] },
        frame: { type: "string", minLength: 1, maxLength: 64 },
        variant: { $ref: "common.schema.json#/$defs/localId" }
      },
      allOf: [
        {
          if: { properties: { profile: { const: "sprite" } }, required: ["profile"] },
          then: { properties: { view: {} }, required: ["view"] },
          else: {
            not: {
              anyOf: [
                { properties: { clip: {} }, required: ["clip"] },
                { properties: { frame: {} }, required: ["frame"] }
              ]
            }
          }
        }
    ...
    fragment: {
      ...
      properties: {
        id: { $ref: "common.schema.json#/$defs/localId" },
        selector: { $ref: "#/$defs/selector" },
        source: { $ref: "common.schema.json#/$defs/safePath" },
        plane: { $ref: "common.schema.json#/$defs/localId" },
        order: { type: "integer", minimum: -10, maximum: 10 },
        anchor: { $ref: "common.schema.json#/$defs/localId" },
        offset: {
          type: "array",
        ...
        bounds: { $ref: "common.schema.json#/$defs/rect" },
        extensions: { $ref: "common.schema.json#/$defs/extensions" }
      }
    }
  },
  ...
  properties: {
    ...
    display: {
      ...
      properties: {
        ...
        thumbnail: { $ref: "common.schema.json#/$defs/safePath" }
      }
    },
    compatibility: {
      type: "object",
      additionalProperties: false,
      required: ["rigFamilies", "engine", "fitTags"],
      properties: {
        rigFamilies: {
          type: "array",
          minItems: 1,
    ...
    equip: {
      type: "object",
      additionalProperties: false,
      required: ["slots", "requires", "conflicts", "provides"],
      properties: {
        slots: {
          type: "array",
          minItems: 1,
        ...
        exclusiveGroup: { $ref: "common.schema.json#/$defs/localId" },
        requires: {
          type: "array",
          uniqueItems: true,
    ...
    palette: {
      ...
      properties: {
        roles: {
          type: "object",
          propertyNames: { $ref: "common.schema.json#/$defs/localId" },
          additionalProperties: {
            type: "object",
            additionalProperties: false,
            required: ["default", "mode"],
            properties: {
              default: { $ref: "common.schema.json#/$defs/hexColor" },
              mode: { enum: ["multiply", "screen", "replace"] }
            }
          }
        }
      }
        ...
        oneOf: [
          {
            type: "object",
            additionalProperties: false,
            required: ["kind", "targetTags"],
            properties: {
              kind: { const: "suppress-tags" },
              targetTags: {
                type: "array",
                minItems: 1,
                uniqueItems: true,
          ...
          },
          {
            type: "object",
            additionalProperties: false,
            required: ["kind", "assetId", "variant"],
            properties: {
              kind: { const: "select-variant" },
    ...
    provenance: {
      type: "object",
      additionalProperties: false,
      required: ["authors", "license", "source", "contentHash"],
      properties: {
        authors: {
          type: "array",
          minItems: 1,
...
// packages/schema/src/schemas/common.schema.json
var common_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://character-creator.dev/schemas/0.1.0/common.schema.json",
  $defs: {
    ...
    semverRange: {
      type: "string",
      pattern: "^(?:[<>=~^]{0,2}[0-9]+\\.[0-9]+\\.[0-9]+(?:-[0-9A-Za-z.-]+)?)(?:\\s+(?:[<>=~^]{0,2}[0-9]+\\.[0-9]+\\.[0-9]+(?:-[0-9A-Za-z.-]+)?))*$",
      maxLength: 128
    },
    contentHash: {
      type: "string",
...
// packages/schema/src/schemas/diagnostic.schema.json
var diagnostic_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://character-creator.dev/schemas/0.1.0/diagnostic.schema.json",
  type: "object",
  additionalProperties: false,
  ...
  properties: {
    ...
    severity: { enum: ["error", "warning", "review-required"] },
...
// packages/schema/src/schemas/recipe.schema.json
var recipe_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://character-creator.dev/schemas/0.1.0/recipe.schema.json",
  type: "object",
  additionalProperties: false,
  required: ["schemaVersion", "engineVersion", "rigFamily", "equipped", "palette", "parameters", "seed"],
  properties: {
    schemaVersion: { $ref: "common.schema.json#/$defs/schemaVersion" },
    engineVersion: { $ref: "common.schema.json#/$defs/semver" },
    rigFamily: { $ref: "common.schema.json#/$defs/rigFamily" },
    equipped: {
      type: "array",
      maxItems: 256,
      items: {
    ...
    palette: {
      type: "object",
      propertyNames: { $ref: "common.schema.json#/$defs/localId" },
      additionalProperties: { $ref: "common.schema.json#/$defs/hexColor" },
      maxProperties: 128
    },
    parameters: {
      type: "object",
    ...
    metadata: {
      type: "object",
      propertyNames: { $ref: "common.schema.json#/$defs/localId" },
      additionalProperties: { type: "string", maxLength: 1e3 },
      maxProperties: 32
    },
    extensions: { $ref: "common.schema.json#/$defs/extensions" }
  }
...
// packages/schema/src/schemas/rig.schema.json
var rig_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://character-creator.dev/schemas/0.1.0/rig.schema.json",
  type: "object",
  additionalProperties: false,
  required: [
    "schemaVersion",
  ...
  properties: {
    schemaVersion: { $ref: "common.schema.json#/$defs/schemaVersion" },
    id: { $ref: "common.schema.json#/$defs/rigFamily" },
    version: { $ref: "common.schema.json#/$defs/semver" },
    coordinateSystem: {
      type: "object",
      additionalProperties: false,
      required: ["origin", "units", "yAxis"],
      properties: {
    ...
    profiles: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["id", "width", "height", "views", "requiredCoverage", "safeArea", "sampling"],
        properties: {
          id: { enum: ["portrait", "full-body", "sprite"] },
    ...
    localOrder: {
      ...
      required: ["min", "max"],
      properties: {
        min: { type: "integer", minimum: -100, maximum: 0 },
    ...
    slots: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["id", "exclusive", "allowedKinds"],
        properties: {
          id: { $ref: "common.schema.json#/$defs/localId" },
    ...
    regions: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["id", "anatomyCritical"],
        properties: {
          id: { $ref: "common.schema.json#/$defs/localId" },
          anatomyCritical: { type: "boolean" }
        }
      }
    },
    anchors: {
      ...
      items: {
        ...
        properties: {
          ...
          profiles: {
            ...
            minItems: 1,
            uniqueItems: true,
            items: { enum: ["portrait", "full-body", "sprite"] }
          }
        }
      }
    },
    expressions: {
      ...
      items: {
        ...
        properties: {
          ...
          channels: {
            type: "object",
            additionalProperties: { $ref: "common.schema.json#/$defs/localId" }
          }
        }
      }
    },
    clips: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["id", "loop", "directions", "frames"],
        properties: {
          id: { enum: ["idle", "sit", "walk", "run"] },
          loop: { type: "boolean" },
          directions: {
            type: "array",
            minItems: 1,
          ...
          frames: {
            ...
            items: {
              type: "object",
              additionalProperties: false,
              required: ["id", "durationMs", "contacts"],
              properties: {
                id: { $ref: "common.schema.json#/$defs/localId" },
                durationMs: { type: "integer", minimum: 16, maximum: 1e4 },
                contacts: {
                  type: "array",
                  uniqueItems: true,
                  items: { $ref: "common.schema.json#/$defs/localId" }
    ...
    fallbacks: {
      type: "array",
      items: { $ref: "asset.schema.json#/$defs/fallback" }
    },
    masks: {
      type: "array",
      uniqueItems: true,
    ...
    mirroring: {
      type: "object",
      additionalProperties: false,
      required: ["from", "to", "requiresCapability"],
      properties: {
        from: { const: "left" },
        to: { const: "right" },
        requiresCapability: { $ref: "common.schema.json#/$defs/localId" }
      }
    },
    budgets: {
      type: "object",
      additionalProperties: false,
      required: ["maxFragmentsPerAsset", "maxCanvasArea", "maxAssetBytes"],
      properties: {
        maxFragmentsPerAsset: { type: "integer", minimum: 1, maximum: 1e3 },
        maxCanvasArea: { type: "integer", minimum: 1 },
        maxAssetBytes: { type: "integer", minimum: 1 }
      }
    },
    extensions: { $ref: "common.schema.json#/$defs/extensions" }
  }
...
var jsonSchemas = {
  ...
  rig: rig_schema_default,
  asset: asset_schema_default,
  assetPack: asset_pack_schema_default,
  recipe: recipe_schema_default,
  diagnostic: diagnostic_schema_default
};
function isSafeRelativePath(value) {
  if (value.length === 0 || value.startsWith("/") || value.startsWith("\\") || /^[A-Za-z]:/.test(value) || value.includes("\\") || value.includes("\0") || value.includes("%") || value.includes("?") || value.includes("#")) {
    return false;
  }
...
var validators = {
  rig: ajv.getSchema(rig_schema_default.$id),
  asset: ajv.getSchema(asset_schema_default.$id),
  assetPack: ajv.getSchema(asset_pack_schema_default.$id),
  recipe: ajv.getSchema(recipe_schema_default.$id),
  diagnostic: ajv.getSchema(diagnostic_schema_default.$id)
};
function pointerToJsonPath(pointer) {
  if (pointer === "") return "$";
  return pointer.split("/").slice(1).reduce((path, rawSegment) => {
    const segment = rawSegment.replaceAll("~1", "/").replaceAll("~0", "~");
    return /^\d+$/.test(segment) ? `${path}[${segment}]` : `${path}.${segment.replaceAll(".", "\\.")}`;
  }, "$");
}
...
function schemaDiagnostics(errors) {
  return sortDiagnostics(
    (errors ?? []).map(
      (error) => diagnostic(
        error.keyword === "format" && error.params["format"] === "safe-relative-path" ? "INVALID_SAFE_PATH" : "SCHEMA_INVALID",
...
function findUnsafeObjectKey(value, path = "$", seen = /* @__PURE__ */ new Set()) {
  ...
  for (const key of Object.keys(value)) {
    ...
    diagnostics2.push(
      ...findUnsafeObjectKey(value[key], childPath, seen)
    );
  }
  return diagnostics2;
}
function preflight(value) {
  ...
  if (value !== null && typeof value === "object" && "schemaVersion" in value && value.schemaVersion !== SCHEMA_VERSION) {
    return [
      diagnostic(
        "UNSUPPORTED_SCHEMA_VERSION",
        "$.schemaVersion",
        `Expected schema version ${SCHEMA_VERSION}`,
        { details: { received: value.schemaVersion } }
      )
    ];
  }
  return [];
}
function runSchema(value, validator) {
...
function duplicateIdDiagnostics(values, path) {
...
function satisfiesSemverRange(version, range) {
  return range.split(/\s+/).every((comparator) => {
    ...
    return operator === "^" ? versionMajor === major && comparison >= 0 : versionMajor === major && versionMinor === minor && comparison >= 0;
  });
}
function assetSemanticDiagnostics(asset, rig2) {
  const diagnostics2 = [];
  const profiles = new Map(rig2.profiles.map((profile) => [profile.id, profile]));
  const slots = new Map(rig2.slots.map((slot) => [slot.id, slot]));
  const regions = new Set(rig2.regions.map((region) => region.id));
  const anchors = new Set(rig2.anchors.map((anchor) => anchor.id));
  const expressions = new Set(rig2.expressions.map((expression) => expression.id));
  const clips = new Map(rig2.clips.map((clip) => [clip.id, clip]));
  const planes = new Set(rig2.planes);
  const masks = new Set(rig2.masks);
  if (!asset.compatibility.rigFamilies.includes(rig2.id)) {
    diagnostics2.push(
      diagnostic(
        "INCOMPATIBLE_RIG",
        "$.compatibility.rigFamilies",
        `Asset ${asset.id} does not support rig ${rig2.id}`,
        { assetId: asset.id }
      )
    );
  }
  ...
  asset.fragments.forEach((fragment, fragmentIndex) => {
    ...
    } else if (fragment.selector.view !== void 0 && fragment.selector.view !== "*" && !profile.views.includes(fragment.selector.view)) {
      diagnostics2.push(
        diagnostic(
          "UNKNOWN_VIEW",
          `${path}.selector.view`,
    ...
    if (fragment.order < rig2.localOrder.min || fragment.order > rig2.localOrder.max) {
      diagnostics2.push(
        diagnostic(
          "SCHEMA_INVALID",
          `${path}.order`,
          `Local order must be within ${rig2.localOrder.min}..${rig2.localOrder.max}`,
          { assetId: asset.id }
        )
      );
    }
    ...
    if (fragment.selector.expression !== void 0 && fragment.selector.expression !== "*" && !expressions.has(fragment.selector.expression)) {
      diagnostics2.push(
        diagnostic(
          "SCHEMA_INVALID",
    ...
    if (fragment.selector.clip !== void 0) {
      ...
      } else if (fragment.selector.frame !== void 0 && fragment.selector.frame !== "*" && !clip.frames.some((frame) => frame.id === fragment.selector.frame)) {
        diagnostics2.push(
          diagnostic(
            "UNKNOWN_FRAME",
            `${path}.selector.frame`,
  ...
  if (asset.fragments.length > rig2.budgets.maxFragmentsPerAsset) {
    diagnostics2.push(
      diagnostic(
        "SCHEMA_INVALID",
        "$.fragments",
        `Asset exceeds rig fragment budget of ${rig2.budgets.maxFragmentsPerAsset}`,
        { assetId: asset.id }
      )
    );
  }
  diagnostics2.push(...duplicateIdDiagnostics(asset.fragments, "$.fragments"));
  return sortDiagnostics(diagnostics2);
}
function parseRig(value) {
  const parsed = runSchema(value, validators.rig);
  if (!parsed.ok) return parsed;
  const diagnostics2 = rigSemanticDiagnostics(parsed.value);
  return diagnostics2.length > 0 ? { ok: false, diagnostics: diagnostics2 } : { ok: true, value: normalizeRig(parsed.value), diagnostics: [] };
}
function parseAssetManifest(value, rig2) {
  const parsed = runSchema(value, validators.asset);
  if (!parsed.ok) return parsed;
  const diagnostics2 = rig2 === void 0 ? [] : assetSemanticDiagnostics(parsed.value, rig2);
  return diagnostics2.length > 0 ? { ok: false, diagnostics: diagnostics2 } : { ok: true, value: normalizeAsset(parsed.value), diagnostics: [] };
}
function parseCharacterRecipe(value) {
  const parsed = runSchema(value, validators.recipe);
  return parsed.ok ? { ok: true, value: normalizeRecipe(parsed.value), diagnostics: [] } : parsed;
}
function validateRecipeSelection(recipe, assets2, rig2) {
  const diagnostics2 = [];
  if (recipe.rigFamily !== rig2.id) {
    diagnostics2.push(
      diagnostic(
        "INCOMPATIBLE_RIG",
        "$.rigFamily",
        `Recipe targets ${recipe.rigFamily}, expected ${rig2.id}`
      )
    );
  }
  const byId2 = new Map(assets2.map((asset) => [asset.id, asset]));
  const equipped2 = recipe.equipped.map((selection, index) => ({ selection, index, asset: byId2.get(selection.assetId) })).filter(
    (item) => {
      if (item.asset !== void 0) return true;
      diagnostics2.push(
        diagnostic(
          "ASSET_NOT_FOUND",
  ...
  for (const { selection, index, asset } of equipped2) {
    if (selection.version !== void 0 && selection.version !== asset.version) {
      diagnostics2.push(
        diagnostic(
          "ASSET_NOT_FOUND",
  ...
  const slotOwners = /* @__PURE__ */ new Map();
  for (const { asset } of equipped2) {
  ...
  for (const slot of rig2.slots.filter((candidate) => candidate.exclusive)) {
...
function createCatalog(assets2) {
  const orderedAssets = [...assets2].sort(
  ...
  return {
    ...
    diagnostics: sortDiagnostics(diagnostics2)
  };
}
function isAssetCatalog(value) {
...
function resolveRequest(request, rig2) {
  const diagnostics2 = [];
  const profile = rig2.profiles.find((candidate) => candidate.id === request.profile);
  const fallbackProfile = rig2.profiles[0];
  if (profile === void 0) {
    diagnostics2.push(
  ...
  if (selectedProfile === void 0) throw new Error("A validated rig must define a profile");
  ...
  if (request.variant !== void 0) resolved.variant = request.variant;
  if (request.profile === "sprite") {
    const clipId = request.clip ?? "idle";
    const clip = rig2.clips.find((candidate) => candidate.id === clipId);
    if (clip === void 0) {
      diagnostics2.push(diagnostic("UNKNOWN_CLIP", "$.request.clip", `Unknown clip ${clipId}`));
    }
    const frame = request.frame ?? clip?.frames[0]?.id ?? "center";
    if (clip !== void 0 && !clip.frames.some((candidate) => candidate.id === frame)) {
      diagnostics2.push(
        diagnostic(
          "UNKNOWN_FRAME",
          "$.request.frame",
  ...
  } else if (request.clip !== void 0 || request.frame !== void 0) {
    diagnostics2.push(
      diagnostic(
        "UNKNOWN_CLIP",
        "$.request.clip",
  ...
  return { request: resolved, diagnostics: diagnostics2 };
}
function fallbackDistance(axis, from, to, asset, rig2) {
  const fallbacks = [...rig2.fallbacks, ...asset.fallbacks].filter(
    (fallback) => fallback.axis === axis && fallback.allowedFor.includes(asset.kind) && !fallback.forbiddenFor.includes(asset.kind)
  );
  const queue = [{ value: from, distance: 0 }];
  const visited = /* @__PURE__ */ new Set();
  while (queue.length > 0) {
    const current = queue.shift();
    if (current === void 0 || visited.has(current.value)) continue;
...
function selectorFieldScore(axis, selector, requested, asset, rig2) {
  if (selector === void 0 || selector === "*") return 0;
  if (requested === void 0) return void 0;
  if (selector === requested) return 100;
  const distance = fallbackDistance(axis, requested, selector, asset, rig2);
  return distance === void 0 ? void 0 : 50 - distance;
}
function selectorScore(fragment, request, asset, rig2) {
  if (fragment.selector.profile !== request.profile) return void 0;
  const scores = [
    selectorFieldScore("view", fragment.selector.view, request.view, asset, rig2),
    selectorFieldScore(
      "expression",
    ...
    selectorFieldScore("clip", fragment.selector.clip, request.clip, asset, rig2),
    selectorFieldScore("frame", fragment.selector.frame, request.frame, asset, rig2),
    selectorFieldScore("variant", fragment.selector.variant, request.variant, asset, rig2)
  ];
  if (scores.some((score) => score === void 0)) return void 0;
  return scores.reduce((total, score) => total + score, 0);
}
function fragmentLogicalKey(fragment) {
  const tags = [...fragment.tags].sort().join("|");
  return `${fragment.plane}:${tags || fragment.id}`;
}
function selectFragments(asset, request, rig2) {
  const scores = /* @__PURE__ */ new Map();
  for (const fragment of asset.fragments) {
    const score = selectorScore(fragment, request, asset, rig2);
    if (score !== void 0) scores.set(fragment, score);
  }
  ...
  for (const group of groups.values()) {
    const ranked = [...group].sort(
      (left, right) => (scores.get(right) ?? 0) - (scores.get(left) ?? 0) || left.id.localeCompare(right.id)
    );
    ...
    if (tied.length > 1) {
      diagnostics2.push(
        diagnostic(
          "AMBIGUOUS_FRAGMENT",
          "$",
          `Asset ${asset.id} has equally specific fragments: ${tied.map((fragment) => fragment.id).join(", ")}`,
  ...
  return { fragments: selected, diagnostics: diagnostics2 };
}
function validateRequirementsAndConflicts(equipped2, rig2) {
  const diagnostics2 = [];
  const equippedIds = new Set(equipped2.map((asset) => asset.id));
  const capabilities = new Set(
    rig2.anchors.map((anchor) => `anchor:${anchor.id}`)
  );
  for (const asset of equipped2) {
  ...
  for (const asset of equipped2) {
    for (const requirement of asset.equip.requires) {
      const satisfied = requirement.startsWith("asset:") ? equippedIds.has(requirement.slice("asset:".length)) : capabilities.has(requirement);
      if (!satisfied) {
        diagnostics2.push(
          diagnostic(
            "MISSING_REQUIREMENT",
  ...
  const emittedConflicts = /* @__PURE__ */ new Set();
  for (const asset of equipped2) {
    for (const conflict of asset.equip.conflicts) {
      const target = conflict.startsWith("asset:") ? conflict.slice("asset:".length) : conflict;
      const active = conflict.startsWith("asset:") ? equippedIds.has(target) : capabilities.has(conflict);
      if (!active) continue;
...
function buildProvenance(recipe, rig2, request, equipped2) {
  return {
    engineVersion: ENGINE_VERSION,
    schemaVersion: SCHEMA_VERSION,
    rig: { id: rig2.id, version: rig2.version },
    recipeFingerprint: fingerprint(recipe),
    request,
    assets: [...equipped2].sort((left, right) => left.id.localeCompare(right.id)).map((asset) => ({
      id: asset.id,
...
function emptyScene(rig2, request, diagnostics2, recipe, equipped2) {
  const profile = rig2.profiles.find((candidate) => candidate.id === request.profile) ?? rig2.profiles[0];
  if (profile === void 0) throw new Error("A validated rig must define a profile");
  return {
    width: profile.width,
    ...
    drawList: [],
    diagnostics: sortDiagnostics(diagnostics2),
    provenance: buildProvenance(recipe, rig2, request, equipped2)
  };
}
function resolveCharacter(input) {
  ...
  diagnostics2.push(...requestResult.diagnostics);
  ...
  const equipped2 = recipe.equipped.map((selection) => catalog.assets.get(selection.assetId)).filter((asset) => asset !== void 0);
  diagnostics2.push(...validateRequirementsAndConflicts(equipped2, input.rig));
  if (diagnostics2.some((item) => item.severity === "error")) {
    return emptyScene(
      input.rig,
  ...
  for (const asset of equipped2) {
    ...
    diagnostics2.push(...selection.diagnostics);
    for (const fragment of selection.fragments) selected.push({ asset, fragment });
  }
  const knownTags = new Set(
    equipped2.flatMap((asset) => asset.fragments.flatMap((fragment) => fragment.tags))
  );
  const suppressedTags = /* @__PURE__ */ new Set();
  for (const asset of equipped2) {
  ...
  const drawList = visible.map(({ asset, fragment }) => {
    ...
    (left, right) => left.planeIndex - right.planeIndex || left.order - right.order || left.assetId.localeCompare(right.assetId) || left.fragmentId.localeCompare(right.fragmentId)
  );
  return {
    width: profile.width,
    height: profile.height,
    ...
    drawList,
    diagnostics: sortDiagnostics(diagnostics2),
    provenance: buildProvenance(recipe, input.rig, requestResult.request, equipped2)
  };
}

// packages/core/src/recipes.ts
var DEFAULT_RECIPE_LIMITS = {
  maxBytes: 128 * 1024,
  maxDepth: 32,
  maxObjectKeys: 2048,
  maxArrayItems: 512
...
function canonicalStringify(value) {
  ...
  return `{${Object.entries(value).sort(([left], [right]) => left.localeCompare(right)).map(([key, child]) => `${JSON.stringify(key)}:${canonicalStringify(child)}`).join(",")}}`;
}
function exportCharacterRecipe(recipe) {
  const parsed = parseCharacterRecipe(recipe);
  return parsed.ok ? { ok: true, value: `${canonicalStringify(parsed.value)}
`, diagnostics: [] } : parsed;
}
function inspectStructure(value, limits, state, path = "$", depth = 0) {
  ...
  if (typeof value === "string") {
    if (/^(?:data:image\/|javascript:|file:|https?:\/\/)/i.test(value) || /^[A-Za-z]:[\\/]/.test(value) || value.includes("\\") || /(?:^|\/)\.\.(?:\/|$)/.test(value)) {
      state.diagnostics.push(
        diagnostic(
          "UNSAFE_RECIPE_VALUE",
          path,
...
function parseRecipeJson(sourceText, customLimits = {}) {
  ...
  const state = { keys: 0, items: 0, diagnostics: [] };
  inspectStructure(value, limits, state);
  return state.diagnostics.length > 0 ? { ok: false, diagnostics: sortDiagnostics(state.diagnostics) } : { ok: true, value, diagnostics: [] };
}
var migration001To010 = {
  from: "0.0.1",
  to: SCHEMA_VERSION,
  migrate(value) {
    ...
    return {
      ...
      equipped: legacyAssets.map(
        (asset) => typeof asset === "string" ? { assetId: asset } : asset
      ),
...
var defaultRecipeMigrations = [migration001To010];
function migrateRecipe(input, migrations) {
...
function applyAliases(recipe, aliases) {
  const applied = [];
  const diagnostics2 = [];
  const equipped2 = recipe.equipped.map((selection, index) => {
    const replacement = aliases[selection.assetId];
    if (replacement === void 0 || replacement === selection.assetId) return selection;
  ...
  return { recipe: normalizeRecipe({ ...recipe, equipped: equipped2 }), applied, diagnostics: diagnostics2 };
}
function selectionDiagnostics(recipe, catalog, rig2) {
  if (catalog === void 0 || rig2 === void 0) return { diagnostics: [], assets: [] };
  const resolvedCatalog = isAssetCatalog(catalog) ? catalog : createCatalog(catalog).catalog;
  return {
    diagnostics: validateRecipeSelection(recipe, resolvedCatalog.orderedAssets, rig2),
    assets: resolvedCatalog.orderedAssets
  };
}
function previewRecipeFor(recipe, assets2, diagnostics2) {
  const byId2 = new Map(assets2.map((asset) => [asset.id, asset]));
  const equipped2 = recipe.equipped.filter((selection) => {
    const asset = byId2.get(selection.assetId);
    const usable = asset !== void 0 && (selection.version === void 0 || selection.version === asset.version);
    if (!usable) {
      diagnostics2.push(
        diagnostic(
          "BEST_EFFORT_SUBSTITUTION",
  ...
  return normalizeRecipe({ ...recipe, equipped: equipped2 });
}
function importCharacterRecipe(sourceText, options = {}) {
  ...
  const diagnostics2 = sortDiagnostics([...aliased.diagnostics, ...selection.diagnostics]);
  const errors = diagnostics2.filter((item) => item.severity === "error");
  if (errors.length > 0 && (options.mode ?? "strict") === "strict") {
    return { ok: false, sourceText, diagnostics: diagnostics2 };
  }
  const previewDiagnostics = [...diagnostics2];
  const previewRecipe = errors.length > 0 ? previewRecipeFor(aliased.recipe, selection.assets, previewDiagnostics) : aliased.recipe;
  return {
    ok: true,
    value: {
      sourceText,
...
// packages/creator-ui/src/store.ts
...
var CreatorStore = class {
  ...
  constructor(options) {
    ...
    this.catalog = Array.isArray(options.catalog) ? createCatalog(options.catalog).catalog : options.catalog;
  ...
  #commit(recipe, diagnostics2 = []) {
    ...
    this.#emit();
    return { ok: true, snapshot: this.snapshot, diagnostics: diagnostics2 };
  }
  queryCatalog(query = {}) {
    ...
    return this.catalog.orderedAssets.filter((asset) => {
      if (query.kinds !== void 0 && !query.kinds.includes(asset.kind)) return false;
      if (query.tags !== void 0 && !query.tags.every((tag) => asset.display.tags.includes(tag))) return false;
      if (search2 !== "" && ![asset.display.name, asset.id, ...asset.display.tags].some((value) => value.toLocaleLowerCase().includes(search2))) return false;
  ...
  #candidateWith(asset, recipe) {
    const slots = new Set(asset.equip.slots);
    const equipped2 = recipe.equipped.filter((selection) => {
      const current = this.catalog.assets.get(selection.assetId);
      return current === void 0 || !current.equip.slots.some((slot) => slots.has(slot) && this.rig.slots.find((candidate) => candidate.id === slot)?.exclusive);
    });
    equipped2.push({ assetId: asset.id, version: asset.version });
    return normalizeRecipe({ ...recipe, equipped: equipped2 });
  }
  equip(assetId) {
    const asset = this.catalog.assets.get(assetId);
    if (asset === void 0) {
      const diagnostics2 = [diagnostic("ASSET_NOT_FOUND", "$.equipped", `Unknown asset ${assetId}`, { assetId })];
      this.#diagnostics = diagnostics2;
      this.#emit();
      return { ok: false, snapshot: this.snapshot, diagnostics: diagnostics2 };
    }
  ...
  selectBodyProfile(id) {
    ...
    if (profile === void 0) {
      const diagnostics2 = [diagnostic("ASSET_NOT_FOUND", "$.bodyProfile", `Unknown body profile ${id}`)];
      return { ok: false, snapshot: this.snapshot, diagnostics: diagnostics2 };
    }
...
var canvas = byId("character-canvas");
var assetGrid = byId("asset-grid");
var search = byId("search");
var compatible = byId("compatible");
var categories = byId("categories");
var equipped = byId("equipped");
var paletteControls = byId("palette-controls");
var diagnostics = byId("diagnostics");
var renderState = byId("render-state");
var bodyProfile = byId("body-profile");
var heroRecipesElement = byId("hero-recipes");
var dialog = byId("recipe-dialog");
var recipeJson = byId("recipe-json");
var dialogTitle = byId("dialog-title");
var dialogHelp = byId("dialog-help");
...
function currentRequest() {
  ...
  return request.profile === "sprite" ? { ...request, view: request.view ?? "front", clip: request.clip ?? "idle", frame: request.frame ?? "center" } : request;
}
async function render() {
  ...
  diagnostics.innerHTML = errors.length === 0 ? "<p>No conflicts. This recipe is ready to render.</p>" : errors.map((item) => `<p class="error"><strong>${item.code}</strong> \u2014 ${item.message}</p>`).join("");
}
function updateCatalog() {
  const kinds = activeCategory === "all" ? void 0 : [activeCategory];
...
function updateInspector() {
  ...
  equipped.innerHTML = snapshot.recipe.equipped.map((selection) => {
    ...
    return `<li>${asset?.display.name ?? selection.assetId}<button type="button" data-remove="${selection.assetId}" aria-label="Remove ${asset?.display.name ?? selection.assetId}">\xD7</button></li>`;
  }).join("");
...
function download(name, blob) {
  ...
  anchor.click();
  setTimeout(() => URL.revokeObjectURL(anchor.href), 1e3);
}
async function initialize() {
  ...
  const profiles = data.bodyProfiles.map((value) => {
    const parsed = parseCharacterRecipe(value.recipe);
    if (!parsed.ok) throw new Error(parsed.diagnostics.map((item) => item.message).join("; "));
    return { id: value.id, name: value.name, recipe: parsed.value };
  });
  baseline = structuredClone(heroes[0]?.recipe ?? profiles[0]?.recipe);
  if (baseline === void 0) throw new Error("Starter pack did not publish a baseline recipe");
  ...
  assetGrid.addEventListener("click", (event) => {
    ...
    if (id === void 0) return;
    if (button.getAttribute("aria-pressed") === "true") store.unequip(id);
    else store.equip(id);
  });
  equipped.addEventListener("click", (event) => {
    const button = event.target.closest("[data-remove]");
    const id = button?.dataset["remove"];
    if (id !== void 0) store.unequip(id);
  });
  ...
  compatible.addEventListener("click", () => {
    compatible.setAttribute("aria-pressed", String(compatible.getAttribute("aria-pressed") !== "true"));
    updateCatalog();
  });
  document.querySelectorAll("[data-preview]").forEach((button) => button.addEventListener("click", () => {
    document.querySelectorAll("[data-preview]").forEach((item) => item.setAttribute("aria-pressed", String(item === button)));
    const profile = button.dataset["preview"];
  ...
  heroRecipesElement.addEventListener("click", (event) => {
    ...
    if (hero !== void 0) store.reset(hero.recipe);
  });
  byId("export-recipe").addEventListener("click", () => {
    dialogTitle.textContent = "Export recipe";
    dialogHelp.textContent = "Compact, canonical JSON contains intent only\u2014never pixels.";
    recipeJson.value = store.exportJson();
    byId("apply-recipe").hidden = true;
    byId("copy-recipe").hidden = false;
    dialog.showModal();
  });
  byId("import-recipe").addEventListener("click", () => {
    dialogTitle.textContent = "Import recipe";
    dialogHelp.textContent = "The source is validated before it can replace your current character.";
    recipeJson.value = "";
    byId("apply-recipe").hidden = false;
    byId("copy-recipe").hidden = true;
    dialog.showModal();
    recipeJson.focus();
  });
  byId("copy-recipe").addEventListener("click", () => void navigator.clipboard.writeText(recipeJson.value));
  byId("apply-recipe").addEventListener("click", () => {
    if (store.importJson(recipeJson.value).ok) dialog.close();
  });
  byId("export-png").addEventListener("click", () => canvas.toBlob((blob) => {
    if (blob !== null) download("character.png", blob);
  }, "image/png"));
  renderState.textContent = "Ready";
}
fetch("validation/index.json").then((response) => response.json()).then((data) => {
  byId("review-links").innerHTML = (data.entries ?? []).map((entry) => `<a href="${entry.href}">${entry.task} \xB7 ${entry.title} <small>${entry.status}</small></a>`).join("");
}).catch(() => void 0);
